#ifndef POKEHEARTGOLD_UNK_02034354_H
#define POKEHEARTGOLD_UNK_02034354_H

#include "player_data.h"

void sub_02034354(SaveData *saveData, int a1);
void sub_020343E4(void);
BOOL sub_02034420(void);
void sub_02034434(void);
void sub_02034638(void);
int sub_020346D4(void);
void sub_020346E8(int arg0);
int sub_02034714(int arg0);
int sub_02034730(int arg0);
int sub_02034750(int arg0);
void sub_0203476C(int arg0);
int sub_02034780(void);
int sub_020347A0(void);
PlayerProfile *sub_02034818(u32 a0);
u8 sub_02034870(int arg0);
// The defining TU was matched declaring this itself as int (void); MWCC
// rejects K&R-vs-prototyped redeclaration.
#ifndef UNK_02034354_OWN_DECLS
s32 sub_020348F0();
#endif
void sub_02034AC0(SaveData *saveData, int a1);

#endif // POKEHEARTGOLD_UNK_02034354_H
