...
**IPA header discipline (critical).** MWCC `-ipa file` cascades codegen changes when ANY declaration in a visible header changes — even adding new function declarations for functions the TU never calls. A/B tested: adding declarations to `sound_02004A44.h` without changing existing prototypes still produces different `.o` files. When a decomp corrects prototypes or adds declarations to a shared header, use the **split header pattern**: keep the public header frozen at the callers' matching-time state; the defining `.c` file includes `<name>_internal.h` with correct types. Example: `sound_02004A44.h` (frozen, caller-facing) + `sound_02004A44_internal.h` (correct types for `unk_02004A44.c`). Files matched against a different header state (e.g. upstream files) must NOT include the changed header — use local forward declarations with the types they were matched with. Before merging upstream changes that modify header declarations, A/B test a representative `.o` file to check for IPA cascade.
